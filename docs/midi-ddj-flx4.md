...
| SHIFT + IN/4BEAT, CUE/LOOP CALL ◄ / ► deck 1 / 2 | `0x90`/`0x91` SHIFT+note `0x10`, notes `0x51` / `0x53` | beat loops on a playback deck (M23, ADR-0016): SHIFT + IN/4BEAT drops a 4-beat auto loop — one button, set when idle, exit when running; CUE/LOOP CALL ◄ / ► halve / double the active loop. The `0x51`/`0x53` halve/double bytes are from the Mixxx FLX4 chart (`loop_halve`/`loop_double`); the SHIFT+IN 4-beat gesture is interpolated and the least certain — confirm all three with the monitor (the M23 checklist holds these boxes) |
